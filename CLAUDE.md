...
|---|---|---|
| 1 | Header (nav) | **Correction — Psiog's nav is white, not dark.** White background, navy text nav links, gold underline beneath the active/hovered link (not a color-only change — Psiog uses an actual underline). Logo sits left, links center-right, CTA button far right. This replaces the current navy header — rebuild it, don't just recolor it. **CTA button (superseding "gold filled pill" above)** — matches Psiog's "Contact Us" nav-button interaction: default white background, navy text, small up-right arrow icon inside a solid gold box with **asymmetric corners** (`rounded-tl-md rounded-bl-md rounded-br-md`, `rounded-tr-none` — sharp on the corner the arrow points toward, `rounded-md` on the other three) matching Psiog's actual "Contact Us" icon exactly, not uniform `rounded-md`, next to the label; on hover, the button background becomes navy, text becomes white, the label+icon shift together (`translate-x-1`), and the arrow additionally lifts diagonally inside its box (a "launch" motion, ~200ms ease-out) — the gold box itself stays gold throughout, only the icon moves. Applies to the Header's "Get Started" (desktop nav + mobile menu). **All CTA buttons site-wide use `rounded-xl`, not `rounded-full`** (boxier rounded-rectangle, matching Psiog's actual button proportions) — this includes Hero's two buttons, the homepage Services cards, the Contact page's fast-track CTA, and the contact form's submit button. The shared label+icon "move together" motion and the gold asymmetric-corner icon box also apply to Hero's "Explore Our Programs" and the homepage Services cards' "Explore" links (same sharp-corner-toward-arrow-direction rule — currently all arrows point up-right, so all sharpen the top-right corner; if a future arrow points a different direction, sharpen whichever corner it points toward instead). **Exception:** Hero's "Explore Our Programs" uses its own full navy/white color inversion instead of the gold icon box — default is navy background, white border, white text, white icon box with a navy arrow; on hover every one of those flips (white background, navy border, navy text, navy icon box with a white arrow) — a straight two-tone swap independent of the Header CTA's white→navy hover, which keeps its own gold-box treatment unchanged. |
